    ...
    for slide in prs.slides:
        for shape in _iter_all_shapes(slide.shapes):
            ...
            try:
                ...
                continue  # EMF/WMF gibi Pillow'un açamadığı formatlar

            if pil_img.format not in ("JPEG", "PNG", "BMP", "TIFF", "GIF", "WEBP"):
                continue
...
async def convert_and_reply(
    ...
    try:
        ...
        # 0) Dosya çok büyükse, önce görselleri sıkıştırmayı dene. Bu,
        #    sonraki tüm adımların (font tarama, autofit, parçalama,
        #    LibreOffice dönüştürme) daha küçük bir dosyayla çalışmasını
        #    sağlar — bellek taşması ihtimalini önemli ölçüde azaltır ve
        #    çıkan PDF'in Telegram'ın 50MB gönderme sınırını aşma
        #    ihtimalini de düşürür.
        raw_size_mb = os.path.getsize(input_path) / (1024 * 1024)
        if raw_size_mb > IMAGE_COMPRESS_THRESHOLD_MB and ext in (
            ".pptx", ".pptm", ".potx"
        ):
            ...
            try:
                compressed_count, saved_bytes = await loop.run_in_executor(
                    None, compress_pptx_images, input_path, compressed_path
                )
                if compressed_count > 0 and os.path.exists(compressed_path):
                    ...
                    # İlk tur yeterli olmadıysa (dosya hâlâ büyükse),
                    # daha agresif ayarlarla ikinci bir tur dene.
                    if new_size_mb > IMAGE_SECOND_PASS_THRESHOLD_MB:
                        ...
                        try:
                            ...
                            if count2 > 0 and os.path.exists(compressed_path2):
                                input_path = compressed_path2
                                final_size_mb = os.path.getsize(input_path) / (1024 * 1024)
                                logger.info(
                                    "İkinci tur: %d görsel daha sıkıştırıldı, "
                                    "%.0fMB -> %.0fMB",
                                    count2, new_size_mb, final_size_mb,
                                )
                        except Exception:  # noqa: BLE001
                            logger.exception(
                                "İkinci sıkıştırma turu başarısız, "
                                "birinci tur sonucuyla devam ediliyor"
                            )
            except Exception:  # noqa: BLE001
                logger.exception(
        ...
        raw_size_mb = os.path.getsize(input_path) / (1024 * 1024)
        skip_preprocessing = raw_size_mb > MEMORY_SAFE_PPTX_MB_THRESHOLD

        if skip_preprocessing:
            logger.info(
                "Dosya %.0f MB, bellek-güvenli eşiği (%d MB) aştığı için "
                "font/autofit adımları atlanıyor (parçalama yine de denenecek)",
                raw_size_mb, MEMORY_SAFE_PPTX_MB_THRESHOLD,
            )
            await status_msg.edit_text(
                f"⚠️ Dosya çok büyük ({raw_size_mb:.0f} MB) olduğu için "
                "font/hizalama otomatik düzeltmeleri bu seferlik atlanıyor. "
                "LibreOffice'in yükünü azaltmak için parçalara bölünecek..."
            )

        # 1) Dosyada kullanılan fontları tara; sistemde kurulu olmayan
        #    ama Google Fonts'ta bulunabilenleri otomatik indirip kur.
        #    (Çok büyük dosyalarda bellek taşmasını önlemek için atlanır.)
        if not skip_preprocessing and ext in (".pptx", ".pptm", ".potx"):
            try:
        ...
        #    ÖNEMLİ: python-pptx ile dosyayı bölmek, görselleri render
        #    etmediği (sadece kopyaladığı) için LibreOffice'in TÜM dosyayı
        #    tek seferde render etmesinden çok daha az bellek kullanır.
        #    Bu yüzden çok büyük dosyalarda bile parçalama denenir —
        #    hatta özellikle onlarda, çünkü asıl bellek yükü LibreOffice'in
        #    render aşamasındadır.
        ...
        effective_chunk_size = (
            5 if raw_size_mb > MEMORY_SAFE_PPTX_MB_THRESHOLD else CHUNK_SIZE
        )
